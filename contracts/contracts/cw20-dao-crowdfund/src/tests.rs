...
fn instantiate_msg_factory_with_token(
    dao_addr: Addr,
    fee_manager_addr: Addr,
    cw20_id: u64,
    funding_goal: u64,
...
fn test_aqua() {
    ...
    let instantiate = instantiate_msg_factory_with_token(
        dao_addr.clone(),
    ...
    // check funding token balance before funding
    let funding_token_addr: Addr = app
        .wrap()
        .query_wasm_smart(escrow_addr.clone(), &QueryMsg::FundingTokenAddr {})
        .unwrap();
    println!("{:?}", funding_token_addr);

    let balance: cw20::BalanceResponse = app
        .wrap()
        .query_wasm_smart(
            funding_token_addr.clone().to_string(),
    ...
    // send aqua token to escrow
    let aqua_transfer_msg = cw20::Cw20ExecuteMsg::Send {
        contract: escrow_addr.clone().to_string(),
        amount: Uint128::from(100000u128),
...
//     let funds_raised: FundToken = app
//         .wrap()
